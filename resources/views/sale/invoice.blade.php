    ...
    <div id="receipt-data">
        <div class="centered">
            @if($general_setting->site_logo)
                <img hidden src="{{url('public/logo', $general_setting->site_logo)}}" height="42" width="50" style="margin:10px 0;filter: brightness(0);">
            @endif
        ...
        <p>{{trans('file.Date')}}: {{$lims_sale_data->created_at}}<br>
            Invoice #: {{$lims_sale_data->reference_no}}<br>
            {{trans('file.customer')}}: {{$lims_customer_data->name}}<br>
            Mobile Number: {{$lims_customer_data->phone_number}}<br>
