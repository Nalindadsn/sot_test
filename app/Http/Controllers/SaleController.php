...
class SaleController extends Controller
{
    public function index(Request $request)
    {
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            $delivery_mans = DeliveryMan::where('is_active', true)->get();

            return view('sale.index',compact('starting_date', 'ending_date', 'warehouse_id', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission','delivery_mans'));
        }
        else
...
//                    ->orderBy($order,$dir)
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                if ($sale->sales_source != null && $sale->sales_source != '')
                {

                    $s_source = SalesSource::find($sale->sales_source);
                    $sale_source =  $s_source->name;
                    $nestedData['sales_source'] = $s_source->name;
                }else{
                    $nestedData['sales_source'] = 'Not Define';
                    $sale_source =  ' Not Define';
                }
                ...
                if($sale->payment_status == 1)
                    $nestedData['payment_status'] = '<div class="badge badge-danger">Cash</div>';
                elseif($sale->payment_status == 2)
                    $nestedData['payment_status'] = '<div class="badge badge-success">Gift Card</div>';
                elseif($sale->payment_status == 3)
                    $nestedData['payment_status'] = '<div class="badge badge-success">Credit Card</div>';
                elseif($sale->payment_status == 4)
                    $nestedData['payment_status'] = '<div class="badge badge-warning"> Cheque </div>';
                elseif($sale->payment_status == 5)
                    $nestedData['payment_status'] = '<div class="badge badge-success"> Paypal </div>';
                elseif($sale->payment_status == 6)
                    $nestedData['payment_status'] = '<div class="badge badge-warning"> Deposit </div>';
                elseif($sale->payment_status == 7)
                    $nestedData['payment_status'] = '<div class="badge badge-danger"> COD </div>';
//                else
//                    $nestedData['payment_status'] = '<div class="badge badge-success">'.trans('file.Paid').'</div>';
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            $sales_sources = SalesSource::where('is_active', true)->get();
            return view('sale.create',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_customer_group_all','sales_sources'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
...
//        return dd($request->sales_source);


        $data['sales_source'] = $request->sales_source;
...
//                $data['reference_no'] = 'posr-' . date("Ymd") . '-'. date("his");
                $sales_count = 100000 + Sale::max('id') +1;
            $data['reference_no'] = 'SOT' .$sales_count;
...
//                $data['reference_no'] = 'sr-' . date("Ymd") . '-'. date("his");
                $sales_count = 1000 + Sale::max('id') +1;
            $data['reference_no'] = 'SOT' .$sales_count;
        }
        ...
        $lims_sale_data->created_at = $request->invoice_date;
        $lims_sale_data->save();
//        $data['created_at'] = $request->invoice_date;
        ...
        $commition = ($request->grand_total * ($biller->commission_pre / 100)) ;
        $new_commition = number_format((float)($biller->commission + $commition), 2, '.', '');


        $biller_commition = new BillersCommision();
        $biller_commition->biller_id = $biller->id;
        $biller_commition->sales_type = 'sales';
        $biller_commition->sales_id = $lims_sale_data->id ;
        $biller_commition->current_commision = $biller->commission;
        $biller_commition->commision_pre = $biller->commission_pre;
        $biller_commition->commision = $commition;
        $biller_commition->new_commision = $new_commition;
        $biller_commition->save();


//        $biller->commission = $biller->commission + $commition;
        $biller->commission = $new_commition;
        $biller->sales_achived = $biller->sales_achived + 1;
        $biller->save();
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            return view('sale.pos', compact('all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_product_list', 'product_number', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_brand_list', 'lims_category_list', 'recent_sale', 'recent_draft', 'lims_coupon_list', 'flag','sales_sources'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
...
//        $data['reference_no'] = 'sr-' . date("Ymd") . '-'. date("his");
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('sales-edit')){
            ...
            $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
            $sales_sources = SalesSource::where('is_active', true)->get();
            return view('sale.edit',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_sale_data','lims_product_sale_data','sales_sources'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $lims_sale_data->payment_status = $request->payment_status;
    ...
    public function genInvoice($id)
    {
        ...
        if ($lims_customer_data->customer_group_id == 1){
            ...
            return view('sale.invoice1', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
        }
    }
