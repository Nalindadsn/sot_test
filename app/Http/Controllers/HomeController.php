...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $sales_sources = SalesSource::where('is_active','=',1)->get();

            foreach ($sales_sources as $sales_source){
                $sales_source['tot_sale'] = Sale::where('sales_source','=',$sales_source->id)
                    ->whereDate('created_at', '>=' , $start_date)
                    ->whereDate('created_at', '<=' , $end_date)
                    ->sum('grand_total');
                $sales_source['sales_count'] = Sale::where('sales_source','=',$sales_source->id)
                    ->whereDate('created_at', '>=' , $start_date)
                    ->whereDate('created_at', '<=' , $end_date)
                    ->count();
            }
...
//        return $sales_sources;
        return view('index', compact('revenue', 'purchase', 'expense', 'return', 'purchase_return', 'profit', 'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount', 'recent_sale', 'recent_purchase', 'recent_quotation', 'recent_payment', 'best_selling_qty', 'yearly_best_selling_qty', 'yearly_best_selling_price','sales_sources'));
    }
